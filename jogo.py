...
while jogo:
    ...
    screen.blit(imagem, (0, 0))  # Colocando a imagem como o fundo
    # gerando os retangulos do mapa na tela
    for i in range(len(mapas.mapa_1)):
        a = mapas.mapa_1[i][0]
        b = mapas.mapa_1[i][1]
        c = mapas.mapa_1[i][2]
        d = mapas.mapa_1[i][3]
    ...
    if mudanca == 0:
        morcego_frente.blit(screen, int(FrameAtualMorcegoFrente), (255, y),
                            Origin.TopLeft)  # inserindo o gif de morcego na tela
        y += 1

    elif mudanca == 1:  # se ocorrer a mudança, ou sjea, chegar no final, o morcego vai voltar de costas
        morcego_tras.blit(screen, int(FrameAtualMorcegoTras), (255, y),
                          Origin.TopLeft)  # inserindo o gif de morcego tras na tela
        y -= 1
